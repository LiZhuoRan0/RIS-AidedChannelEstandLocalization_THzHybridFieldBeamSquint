...
%% -    -   UE2BS
        ...
        Y_absolute_all = (A*H_UE2BS_absolute + sqrt(P_noise)/sqrt(10^(P_t_dBm(i_P_t_dBm)/10))*Noise);
...
%% -    calculate the coordinate  
        ...
        % relative to the BS
        r_est = sqrt((UE_est.x - RIS.x).^2+(UE_est.y-RIS.y).^2);
        theta_est = theta_UE2RIS_fine;   
%% channel estimation and calculate NMSE
        for i_L = 1:length(L_2)
           [H_hat, it_num] = PgOMP_CE_RIS_AS(L_2(i_L), threshold(i_L), Y_RIS, A_RIS, L_max,...
                NRIS, fc, NRF, P_RIS, angleOverSmp, M, B, theta_est, r_est);
