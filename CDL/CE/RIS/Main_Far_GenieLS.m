...
%% channel estimation and calucalte NMSE     
        H_hat_GenieLS = CE_GenieLS_RIS(Y_RIS, A_RIS,...
                    sin(pi/2+angle_RIS-[theta_UE2RIS, Scatter_UE2RIS.theta]), ...
                    [r_UE2RIS, Scatter_UE2RIS.r],...
                    [r_UE2RIS, Scatter_UE2RIS.r_LastHop],...        
                    [1./(r_UE2RIS_All*4*pi) Scatter_UE2RIS.alpha],...
                    fc, B, K); 
        NMSE_GenieLS(i_P_t_dBm) = NMSE_GenieLS(i_P_t_dBm) + (norm(H_UE2RIS_absolute - H_hat_GenieLS, 'fro')/norm(H_UE2RIS_absolute, 'fro')).^2; 
        fprintf("Average NMSE=%ddB\n", 10*log10(NMSE_GenieLS(i_P_t_dBm)/i_it)); 
    end    
