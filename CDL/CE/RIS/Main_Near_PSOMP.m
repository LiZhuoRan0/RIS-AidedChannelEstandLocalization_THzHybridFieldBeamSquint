...
%% channel estimation and calulate the NMSE 
        [H_hat, it_num] = PSOMP_CE_RIS(L_2, threshold, Y_RIS, A_RIS, L_max,...
                   NRIS, fc, NRF, P_RIS, angleOverSmp, M, B);
        NMSE(i_P_t_dBm) = NMSE(i_P_t_dBm) + (norm(H_UE2RIS_absolute - H_hat, 'fro')/norm(H_UE2RIS_absolute, 'fro')).^2;
        Times(i_P_t_dBm) = Times(i_P_t_dBm) + it_num;        
        fprintf('Average NMSE=%ddB; Current NMSE=%ddB\n',10*log10(NMSE(i_P_t_dBm)/i_it), 20*log10(norm(H_UE2RIS_absolute - H_hat, 'fro')/norm(H_UE2RIS_absolute, 'fro')));
        fprintf("Average Itetation Number=%d; Current Iteration Number =%d\n", Times(i_P_t_dBm)/i_it, it_num);       
    end
