function [hat_x, it_num] = gOMP_MMV_CE_replace(L_2, threshold, A, y, L_max)
...
%         end
    ...
    LS = zeros(Q, M);
    for i_M = 1:M
        LS(pos, i_M)=Aug_t(:, :, i_M)\y(:, i_M);          
        r0(:, i_M)=y(:, i_M)-Aug_t(:, :, i_M)*LS(pos, i_M);                            
    end
    
%     if norm(r0, 'fro')/r_norm > threshold || (L_2 ~= 1 && times>=7 && norm(r0, 'fro')/r_norm < 0.8 && norm(r0, 'fro')/r_norm > 0.7)
    if norm(r0, 'fro')/r_norm > threshold || (L_2 ~= 1 && times>=7 && norm(r0, 'fro')/r_norm > 0.7)   
