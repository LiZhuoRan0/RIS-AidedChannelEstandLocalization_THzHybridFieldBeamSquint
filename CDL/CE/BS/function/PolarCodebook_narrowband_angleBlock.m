function [Wfinal, s_final, Z_Delta] = PolarCodebook_narrowband_angleBlock(AngleRange, N,...
                            d, lambda_c, angleOverSmp, M, B)
%%  narrow band dictionary                      
% Inputs:      
    ...
    for n = 1:ceil(N*angleOverSmp*AngleRange)% angle
        ...
        for s = 0:s_final-1% distance          
            if s == 0% far-field case
                a = genb(theta(n), 100, N, fc, d);
                W_angle = [W_angle a];              
            else
                r(n, s+1) = 1/s*Z_Delta*(1 - theta(n)*theta(n));              
                b = genb(theta(n), r(n, s+1), N, fc, d);
%                 b = genb(theta(n), r(n, s+1), NRIS, fc);
                W_angle = [W_angle b];
            end        
        end
        W = [W W_angle];
