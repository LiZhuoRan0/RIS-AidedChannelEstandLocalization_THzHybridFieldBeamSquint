...
%%
NAll = [512];
RMSE.x_co                      = zeros(length(NAll), Nit);
RMSE.y_co                      = zeros(length(NAll), Nit);
RMSE.r_co                      = zeros(length(NAll), Nit);
RMSE.theta_co                  = zeros(length(NAll), Nit);

RMSE.x                      = zeros(length(NAll), Nit);
RMSE.y                      = zeros(length(NAll), Nit);
RMSE.r                      = zeros(length(NAll), Nit);
RMSE.theta                  = zeros(length(NAll), Nit);
%% -    -   scatterers
num_scatter_UE2BS = 3;
...
for i_N = 1:length(NAll)
    ...
    %             10*log10(10.^(P_t_dBm/10).*norm(Y_RIS,'fro')^2./norm(sqrt(P_noise)*Noise,'fro')^2)
        ...
        %% calculate RMSE
        RMSE.theta_co(i_N, i_it) = norm(asin(theta_UE2BS_coarse)+angle_BS-pi/2 - theta_UE2BS);
        RMSE.x_co(i_N, i_it) = norm(UE_est_co.x - UE.x);
        RMSE.y_co(i_N, i_it) = norm(UE_est_co.y - UE.y);
        RMSE.r_co(i_N, i_it) = norm(sqrt(UE.x^2 + UE.y^2) - ...
            sqrt(UE_est_co.x^2 + UE_est_co.y^2));
        fprintf('RMSE.x_co = %10.8f\n', RMSE.x_co(i_N, i_it));
        fprintf('RMSE.y_co = %10.8f\n', RMSE.y_co(i_N, i_it));
        fprintf('RMSE.r_co = %10.8f\n', RMSE.r_co(i_N, i_it));
        fprintf('RMSE.theta_co = %10.8f\n', RMSE.theta_co(i_N, i_it));
    
        RMSE.theta(i_N, i_it) = norm(asin(theta_UE2BS_fine)+angle_BS-pi/2 - theta_UE2BS);
        RMSE.x(i_N, i_it) = norm(UE_est.x - UE.x);
        RMSE.y(i_N, i_it) = norm(UE_est.y - UE.y);
        RMSE.r(i_N, i_it) = norm(sqrt(UE.x^2 + UE.y^2) - ...
