...
P_BS_All                           = [32];% number observed time slots
P_RIS_All                           = [64];
...
P_t_dBm = 30;%dBmW
%%
RMSE.x_co                      = zeros(length(P_BS_All), Nit);
RMSE.y_co                      = zeros(length(P_BS_All), Nit);
RMSE.r_co                      = zeros(length(P_BS_All), Nit);
RMSE.theta_co                  = zeros(length(P_BS_All), Nit);

RMSE.x                      = zeros(length(P_BS_All), Nit);
RMSE.y                      = zeros(length(P_BS_All), Nit);
RMSE.r                      = zeros(length(P_BS_All), Nit);
RMSE.theta                  = zeros(length(P_BS_All), Nit);
%% -    -   scatterers
num_scatter_UE2BS = 3;
...
for i_P_BS = 1:length(P_BS_All)
    P_BS = P_BS_All(i_P_BS);
    P_RIS = P_BS_All(i_P_BS);
...
%% calculate RMSE
        RMSE.theta_co(i_P_BS, i_it) = norm(asin(theta_UE2BS_coarse)+angle_BS-pi/2 - theta_UE2BS);
        RMSE.x_co(i_P_BS, i_it) = norm(UE_est_co.x - UE.x);
        RMSE.y_co(i_P_BS, i_it) = norm(UE_est_co.y - UE.y);
        RMSE.r_co(i_P_BS, i_it) = norm(sqrt(UE.x^2 + UE.y^2) - ...
                        sqrt(UE_est_co.x^2 + UE_est_co.y^2));
        fprintf('RMSE.x_co = %10.8f\n', RMSE.x_co(i_P_BS, i_it));
        fprintf('RMSE.y_co = %10.8f\n', RMSE.y_co(i_P_BS, i_it));
        fprintf('RMSE.r_co = %10.8f\n', RMSE.r_co(i_P_BS, i_it));        
        fprintf('RMSE.theta_co = %10.8f\n', RMSE.theta_co(i_P_BS, i_it));
        
        RMSE.theta(i_P_BS, i_it) = norm(asin(theta_UE2BS_fine)+angle_BS-pi/2 - theta_UE2BS);
        RMSE.x(i_P_BS, i_it) = norm(UE_est.x - UE.x);
        RMSE.y(i_P_BS, i_it) = norm(UE_est.y - UE.y);
        RMSE.r(i_P_BS, i_it) = norm(sqrt(UE.x^2 + UE.y^2) - ...
                        sqrt(UE_est.x^2 + UE_est.y^2));
        fprintf('RMSE.x = %10.8f\n', RMSE.x(i_P_BS, i_it));
        fprintf('RMSE.y = %10.8f\n', RMSE.y(i_P_BS, i_it));
        fprintf('RMSE.r = %10.8f\n', RMSE.r(i_P_BS, i_it));        
        fprintf('RMSE.theta = %10.8f\n', RMSE.theta(i_P_BS, i_it));               
