...
P_t_dBm = 30;%dBmW
%%
RMSE.x_co                      = zeros(length(NRIS_All), Nit);
RMSE.y_co                      = zeros(length(NRIS_All), Nit);
RMSE.r_co                      = zeros(length(NRIS_All), Nit);
RMSE.theta_co                  = zeros(length(NRIS_All), Nit);

RMSE.x                      = zeros(length(NRIS_All), Nit);
RMSE.y                      = zeros(length(NRIS_All), Nit);
RMSE.r                      = zeros(length(NRIS_All), Nit);
RMSE.theta                  = zeros(length(NRIS_All), Nit);
for i_NRIS = 1:length(NRIS_All)
%% -    location setting of BS，RIS，UE, scatter
    % RIS location
    NRIS            =           NRIS_All(i_NRIS);                % number of RIS elements
    S_eff           =           (NRIS*d)^2;
    RIS.x                          = 20*sqrt(2);
    RIS.y                          = 0;
    ...
    %             10*log10(10.^(P_t_dBm/10).*norm(Y_RIS,'fro')^2./norm(sqrt(P_noise)*Noise,'fro')^2)
        ...
        %% calculate RMSE
        RMSE.theta_co(i_NRIS ,i_it) = norm(asin(theta_UE2BS_coarse)+angle_BS-pi/2 - theta_UE2BS);
        RMSE.x_co(i_NRIS ,i_it) = norm(UE_est_co.x - UE.x);
        RMSE.y_co(i_NRIS ,i_it) = norm(UE_est_co.y - UE.y);
        RMSE.r_co(i_NRIS ,i_it) = norm(sqrt(UE.x^2 + UE.y^2) - ...
            sqrt(UE_est_co.x^2 + UE_est_co.y^2));
        fprintf('RMSE.x_co = %10.8f\n', RMSE.x_co(i_NRIS ,i_it));
        fprintf('RMSE.y_co = %10.8f\n', RMSE.y_co(i_NRIS ,i_it));
        fprintf('RMSE.r_co = %10.8f\n', RMSE.r_co(i_NRIS ,i_it));
        fprintf('RMSE.theta_co = %10.8f\n', RMSE.theta_co(i_NRIS , i_it));
    
        RMSE.theta(i_NRIS ,i_it) = norm(asin(theta_UE2BS_fine)+angle_BS-pi/2 - theta_UE2BS);
        RMSE.x(i_NRIS ,i_it) = norm(UE_est.x - UE.x);
        RMSE.y(i_NRIS ,i_it) = norm(UE_est.y - UE.y);
        RMSE.r(i_NRIS ,i_it) = norm(sqrt(UE.x^2 + UE.y^2) - ...
            sqrt(UE_est.x^2 + UE_est.y^2));
        fprintf('RMSE.x = %10.8f\n', RMSE.x(i_NRIS ,i_it));
        fprintf('RMSE.y = %10.8f\n', RMSE.y(i_NRIS ,i_it));
        fprintf('RMSE.r = %10.8f\n', RMSE.r(i_NRIS ,i_it));
        fprintf('RMSE.theta = %10.8f\n', RMSE.theta(i_NRIS ,i_it));
