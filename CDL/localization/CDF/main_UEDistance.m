...
UEx_All = [10   15];
UEy_All = [-5  -7.5];
RMSE.x_co                      = zeros(length(UEx_All), Nit);
RMSE.y_co                      = zeros(length(UEx_All), Nit);
RMSE.r_co                      = zeros(length(UEx_All), Nit);
RMSE.theta_co                  = zeros(length(UEx_All), Nit);

RMSE.x                      = zeros(length(UEx_All), Nit);
RMSE.y                      = zeros(length(UEx_All), Nit);
RMSE.r                      = zeros(length(UEx_All), Nit);
RMSE.theta                  = zeros(length(UEx_All), Nit);
for i_UE = 1:length(UEx_All)
    ...
    for i_it = 1:Nit
        ...
        Y_absolute_all_NoNoise = A*H_UE2BS_absolute;
    
        Noise = (1/sqrt(2)*randn(size(A, 1), M) + 1j/sqrt(2)*randn(size(A, 1), M));
        
        Y_absolute_all = (Y_absolute_all_NoNoise + sqrt(P_noise)/sqrt(10^(P_t_dBm/10))/sqrt(10^(AntGain/10))*Noise);
        %% -    -   -   onvert absolute phase to the relative phase
    ...
    %             10*log10(10.^(P_t_dBm/10).*norm(Y_RIS,'fro')^2./norm(sqrt(P_noise)*Noise,'fro')^2)
        ...
        %% calculate RMSE
        RMSE.theta_co(i_UE, i_it) = norm(asin(theta_UE2BS_coarse)+angle_BS-pi/2 - theta_UE2BS);
        RMSE.x_co(i_UE, i_it) = norm(UE_est_co.x - UE.x);
        RMSE.y_co(i_UE, i_it) = norm(UE_est_co.y - UE.y);
        RMSE.r_co(i_UE, i_it) = norm(sqrt(UE.x^2 + UE.y^2) - ...
            sqrt(UE_est_co.x^2 + UE_est_co.y^2));
        fprintf('RMSE.x_co = %10.8f\n', RMSE.x_co(i_UE, i_it));
        fprintf('RMSE.y_co = %10.8f\n', RMSE.y_co(i_UE, i_it));
        fprintf('RMSE.r_co = %10.8f\n', RMSE.r_co(i_UE, i_it));
        fprintf('RMSE.theta_co = %10.8f\n', RMSE.theta_co(i_UE, i_it));
    
        RMSE.theta(i_UE, i_it) = norm(asin(theta_UE2BS_fine)+angle_BS-pi/2 - theta_UE2BS);
        RMSE.x(i_UE, i_it) = norm(UE_est.x - UE.x);
        RMSE.y(i_UE, i_it) = norm(UE_est.y - UE.y);
        RMSE.r(i_UE, i_it) = norm(sqrt(UE.x^2 + UE.y^2) - ...
            sqrt(UE_est.x^2 + UE_est.y^2));
        fprintf('RMSE.x = %10.8f\n', RMSE.x(i_UE, i_it));
        fprintf('RMSE.y = %10.8f\n', RMSE.y(i_UE, i_it));
        fprintf('RMSE.r = %10.8f\n', RMSE.r(i_UE, i_it));
        fprintf('RMSE.theta = %10.8f\n', RMSE.theta(i_UE, i_it));
