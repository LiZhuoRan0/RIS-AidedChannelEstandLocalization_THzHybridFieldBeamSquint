...
%% localization 
B_All = [100e6 1e9];
RMSE.x_co                      = zeros(length(B_All), Nit);
RMSE.y_co                      = zeros(length(B_All), Nit);
RMSE.r_co                      = zeros(length(B_All), Nit);
RMSE.theta_co                  = zeros(length(B_All), Nit);
...
%     P_noise = 10^(-17.4)*(B);
    ...
    Y_RIS_Noise = Y_RIS + sqrt(P_noise)/sqrt(10^(P_t_dBm/10))*Noise;    
        ...
        %% calculate RMSE
        RMSE.theta_co(i_B, i_it) = norm(asin(theta_UE2BS_coarse)+angle_BS-pi/2 - theta_UE2BS);
        RMSE.x_co(i_B, i_it) = norm(UE_est_co.x - UE.x);
        RMSE.y_co(i_B, i_it) = norm(UE_est_co.y - UE.y);
        RMSE.r_co(i_B, i_it) = norm(sqrt(UE.x^2 + UE.y^2) - ...
            sqrt(UE_est_co.x^2 + UE_est_co.y^2));
        fprintf('RMSE.x_co = %10.8f\n', RMSE.x_co(i_B, i_it));
        fprintf('RMSE.y_co = %10.8f\n', RMSE.y_co(i_B, i_it));
        fprintf('RMSE.r_co = %10.8f\n', RMSE.r_co(i_B, i_it));
        fprintf('RMSE.theta_co = %10.8f\n', RMSE.theta_co(i_B, i_it));
    
        RMSE.theta(i_B, i_it) = norm(asin(theta_UE2BS_fine)+angle_BS-pi/2 - theta_UE2BS);
        RMSE.x(i_B, i_it) = norm(UE_est.x - UE.x);
        RMSE.y(i_B, i_it) = norm(UE_est.y - UE.y);
