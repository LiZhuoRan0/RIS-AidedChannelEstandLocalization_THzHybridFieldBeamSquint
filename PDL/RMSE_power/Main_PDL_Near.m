...
%         r_est_2 = sqrt(UE.x_1_angle^2 + UE.y_1_angle^2);
        ...
        UE.y_1_angle = UE.x_1_angle.*k_B2U;
        %% calculate RMSE
        RMSE.theta(i_P_t_dBm) = RMSE.theta(i_P_t_dBm) + norm(asin(theta_UE2BS_final)+angle_BS-pi/2 - theta_UE2BS).^2;        
        RMSE.x(i_P_t_dBm) = RMSE.x(i_P_t_dBm) + norm(UE.x_1_angle - UE.x)^2;
        RMSE.y(i_P_t_dBm) = RMSE.y(i_P_t_dBm) + norm(UE.y_1_angle - UE.y)^2;
        RMSE.r(i_P_t_dBm) = RMSE.r(i_P_t_dBm) + norm(sqrt(UE.x^2 + UE.y^2) - ...
                        sqrt(UE.x_1_angle^2 + UE.y_1_angle^2))^2;
        ...
        fprintf('current RMSE.r = %10.8f\n', norm(sqrt(UE.x^2 + UE.y^2) - ...
                        sqrt(UE.x_1_angle^2 + UE.y_1_angle^2)));
        fprintf('current RMSE.theta = %10.8f\n', norm(asin(theta_UE2BS_final)+angle_BS-pi/2 - theta_UE2BS));    
   end
end
