...
P_BS_All                           = [16 32];% number observed time slots
P_RIS_All                          = [32 64];
...
%         r_est_2 = sqrt(UE.x_1_angle^2 + UE.y_1_angle^2);
        ...
        UE.y_1_angle = UE.x_1_angle.*k_B2U;
        %% calculate RMSE
        RMSE.theta(i_P_BS, i_it) = norm(asin(theta_UE2BS_final)+angle_BS-pi/2 - theta_UE2BS);        
        RMSE.x(i_P_BS, i_it) = norm(UE.x_1_angle - UE.x);
        RMSE.y(i_P_BS, i_it) = norm(UE.y_1_angle - UE.y);
