...
UExAll = [10 15];
UEyAll = -UExAll/2;
RMSE.x                      = zeros(1, length(UExAll));
RMSE.y                      = zeros(1, length(UExAll));
RMSE.r                      = zeros(1, length(UExAll));
RMSE.theta                  = zeros(1, length(UExAll));

for i_UE = 1:length(UExAll)
...
%%  
    for i_it = 1:Nit
        ...
        m_2 = -2*c;
        m_3 = b*b;     
        UE.x_est = (-m_2+Flag*sqrt(m_2.^2-4.*m_1*m_3))./(2.*m_1);
        ...
        m_2 = -2*c;
        m_3 = b*b;
        UE.x_1_angle = (-m_2+Flag*sqrt(m_2.^2-4.*m_1*m_3))./(2.*m_1);
        ...
        UE.y_1_angle = UE.x_1_angle.*k_B2U;
        %% calculate RMSE
        RMSE.theta(i_UE, i_it) = norm(asin(theta_UE2BS_final)+angle_BS-pi/2 - theta_UE2BS);
        RMSE.x(i_UE, i_it) = norm(UE.x_1_angle - UE.x);
        RMSE.y(i_UE, i_it) = norm(UE.y_1_angle - UE.y);
