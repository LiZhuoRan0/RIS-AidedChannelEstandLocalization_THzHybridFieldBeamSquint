...
%% - RMSE 
RMSE.x_full                      = zeros(1, Nit);
RMSE.y_full                      = zeros(1, Nit);
RMSE.r_full                      = zeros(1, Nit);
RMSE.theta_full                  = zeros(1, Nit);

RMSE.x_full_ESPRIT                      = zeros(1, Nit);
RMSE.y_full_ESPRIT                      = zeros(1, Nit);
RMSE.r_full_ESPRIT                      = zeros(1, Nit);
RMSE.theta_full_ESPRIT                  = zeros(1, Nit);

RMSE.x_hybrid                      = zeros(1, Nit);
RMSE.y_hybrid                       = zeros(1, Nit);
RMSE.r_hybrid                       = zeros(1, Nit);
RMSE.theta_hybrid                  = zeros(1, Nit);
%% -    -   scatterers
num_scatter_UE2BS = 3;
...
%% - Estimate the AoA
   ...
   %% - localization through the AoA and hyperbola
    ...
    RMSE.theta_full(i_it) = norm(asin(theta_UE2BS_final_full)+angle_BS-pi/2 - theta_UE2BS);
    RMSE.theta_full_ESPRIT(i_it) = norm(asin(theta_UE2BS_final_full_ESPRIT)+angle_BS-pi/2 - theta_UE2BS);
    RMSE.theta_hybrid(i_it) = norm(asin(theta_UE2BS_final_hybrid)+angle_BS-pi/2 - theta_UE2BS);
    %% calculate the RMSE of x,y,r
    RMSE.x_full(i_it) = norm(UE.x_1_angle_MUSIC_full - UE.x);
    RMSE.y_full(i_it) = norm(UE.y_1_angle_MUSIC_full - UE.y);
    RMSE.r_full(i_it) = norm(sqrt(UE.x^2 + UE.y^2) - ...
                    sqrt(UE.x_1_angle_MUSIC_full^2 + UE.y_1_angle_MUSIC_full^2));

    RMSE.x_full_ESPRIT(i_it) = norm(UE.x_1_angle_ESPRIT - UE.x);
    RMSE.y_full_ESPRIT(i_it) = norm(UE.y_1_angle_ESPRIT - UE.y);
    RMSE.r_full_ESPRIT(i_it) = norm(sqrt(UE.x^2 + UE.y^2) - ...
                    sqrt(UE.x_1_angle_ESPRIT^2 + UE.y_1_angle_ESPRIT^2));
                
    RMSE.x_hybrid(i_it) = norm(UE.x_1_angle_MUSIC_hybrid - UE.x);
    RMSE.y_hybrid(i_it) = norm(UE.y_1_angle_MUSIC_hybrid - UE.y);
    RMSE.r_hybrid(i_it) = norm(sqrt(UE.x^2 + UE.y^2) - ...
                    sqrt(UE.x_1_angle_MUSIC_hybrid^2 + UE.y_1_angle_MUSIC_hybrid^2));
    
    fprintf('RMSE.x_full = %10.8f\n', RMSE.x_full(1, i_it));
    fprintf('RMSE.y_full = %10.8f\n', RMSE.y_full(1, i_it));
    fprintf('RMSE.r_full = %10.8f\n', RMSE.r_full(1, i_it));
    fprintf('RMSE.theta_full = %10.8f\n', RMSE.theta_full(1, i_it));
    
    fprintf('RMSE.x_full_ESPRIT = %10.8f\n', RMSE.x_full_ESPRIT(1, i_it));
    fprintf('RMSE.y_full_ESPRIT = %10.8f\n', RMSE.y_full_ESPRIT(1, i_it));
    fprintf('RMSE.r_full_ESPRIT = %10.8f\n', RMSE.r_full_ESPRIT(1, i_it));
    fprintf('RMSE.theta_full_ESPRIT = %10.8f\n', RMSE.theta_full_ESPRIT(1, i_it));        
                
    fprintf('RMSE.x_hybrid = %10.8f\n', RMSE.x_hybrid(1, i_it));
    fprintf('RMSE.y_hybrid = %10.8f\n', RMSE.y_hybrid(1, i_it));
    fprintf('RMSE.r_hybrid = %10.8f\n', RMSE.r_hybrid(1, i_it));
    fprintf('RMSE.theta_hybrid = %10.8f\n', RMSE.theta_hybrid(1, i_it));        
end