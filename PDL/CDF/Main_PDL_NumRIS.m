...
Scatter_UE2BS = genScatterUEBS(K, S, N, BS, UE, num_scatter_UE2BS,...                        
                ...
                SizeOfScatter);
%% localization   
NRIS_All               =           [64 128 256 512];                % all the number of RIS elements
...
for i_NumRIS = 1:length(NRIS_All)
    NRIS            =           NRIS_All(i_NumRIS);                % number of RIS elements
    S_eff           =           (NRIS*d)^2;
    % RIS location
    RIS.x                          = 20*sqrt(2);
    RIS.y                          = 0;
    ...
    for i_it = 1:Nit
       ...
       %% localization through the AoA and hyperbola
        ...
        H_1 = NearFieldH(K, N, sin(pi/2-angle_BS+theta_UE2BS_cor_est), r_est, r_est, 1./(r_est_2_All*4*pi).*(10.^(K.*r_est_2_All/10)), fc, B, M, 'relative');
        H_2 = NearFieldH(K, N, theta_UE2BS_final, r_est, r_est, 1./(r_est_2_All*4*pi).*(10.^(K.*r_est_2_All/10)), fc, B, M, 'relative');
        if norm(Y_relative_all-A*H_1, 'fro') < norm(Y_relative_all-A*H_2, 'fro')
        ...
        UE.y_1_angle = UE.x_1_angle.*k_B2U;
        %% calculate RMSE
        RMSE.theta(i_NumRIS, i_it) = norm(asin(theta_UE2BS_final)+angle_BS-pi/2 - theta_UE2BS);
        RMSE.x(i_NumRIS, i_it) = norm(UE.x_1_angle - UE.x);
        RMSE.y(i_NumRIS, i_it) = norm(UE.y_1_angle - UE.y);
        RMSE.r(i_NumRIS, i_it) = norm(sqrt(UE.x^2 + UE.y^2) - ...
            sqrt(UE.x_1_angle^2 + UE.y_1_angle^2));
        fprintf('RMSE.x = %10.6f\n', RMSE.x(i_NumRIS, i_it));
        fprintf('RMSE.y = %10.6f\n', RMSE.y(i_NumRIS, i_it));
        fprintf('RMSE.r = %10.6f\n', RMSE.r(i_NumRIS, i_it));
        fprintf('RMSE.theta = %10.8f\n', RMSE.theta(i_NumRIS, i_it));
    end
end