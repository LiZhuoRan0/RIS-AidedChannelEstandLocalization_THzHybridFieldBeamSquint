...
%%
    ...
    for i_it = 1:Nit
        ...
        UE.y_1_angle = UE.x_1_angle.*k_B2U;
        %% calculate RMSE
        RMSE.theta(i_NumBS, i_it) = norm(asin(theta_UE2BS_final)+angle_BS-pi/2 - theta_UE2BS);
        RMSE.x(i_NumBS, i_it) = norm(UE.x_1_angle - UE.x);
        RMSE.y(i_NumBS, i_it) = norm(UE.y_1_angle - UE.y);
        RMSE.r(i_NumBS, i_it) = norm(sqrt(UE.x^2 + UE.y^2) - ...
            sqrt(UE.x_1_angle^2 + UE.y_1_angle^2));
        fprintf('RMSE.x = %10.6f\n', RMSE.x(i_NumBS, i_it));
        fprintf('RMSE.y = %10.6f\n', RMSE.y(i_NumBS, i_it));
        fprintf('RMSE.r = %10.6f\n', RMSE.r(i_NumBS, i_it));
        fprintf('RMSE.theta = %10.8f\n', RMSE.theta(i_NumBS, i_it));
    end
end