...
d               =           lambda_c/2;
B               =           10e9;               %10GHz
M               =           1;                 %number of carrier frequency
...
%%
RMSE.x_co                      = zeros(1, Nit);
RMSE.y_co                      = zeros(1, Nit);
RMSE.r_co                      = zeros(1, Nit);
RMSE.theta_co                  = zeros(1, Nit);

RMSE.x                      = zeros(1, Nit);
RMSE.y                      = zeros(1, Nit);
RMSE.r                      = zeros(1, Nit);
RMSE.theta                  = zeros(1, Nit);
%% -    -   scatterers
num_scatter_UE2BS = 3;
...
H_UE2BS_relative = NearFieldH(K, N, sin(pi/2-angle_BS+[theta_UE2BS, Scatter_UE2BS.theta])...
    , [r_UE2BS, Scatter_UE2BS.r], [r_UE2BS, Scatter_UE2BS.r_LastHop], [1./(r_UE2BS_All*4*pi).*(10.^(K.*r_UE2BS_All/10)) Scatter_UE2BS.alpha], fc, B, M, 'relative');
...
% theta_All = linspace(sin(pi/2-angle_BS+theta_UE2BS)-0.0001, sin(pi/2-angle_BS+theta_UE2BS)+0.0001, 100);
% r_All = linspace(r_UE2BS-0.05, r_UE2BS+0.05, 100);
theta_All = sin(pi/2-angle_BS+theta_UE2BS)-0.1:0.01:sin(pi/2-angle_BS+theta_UE2BS)+0.1;
r_All = r_UE2BS-10:0.1:r_UE2BS+10;
Loss = zeros(length(theta_All), length(r_All), 2);
...
for i_theta = 1:length(theta_All)
    if mod(i_theta, 10) == 1
        fprintf('i_theta/length(theta_All) = %d/%d\n', i_theta, length(theta_All));
    end
    for i_r = 1:length(r_All)
        UE.x = r_All(i_r)*cos(asin(theta_All(i_theta)) + angle_BS - pi/2);
        UE.y = r_All(i_r)*sin(asin(theta_All(i_theta)) + angle_BS - pi/2);
        r_UE2BS_All = sqrt((UE.x-BS.xAll).^2 +...
                            (UE.y-BS.yAll).^2); 
        H_UE2BS_absolute = NearFieldH(K, N, theta_All(i_theta)...
        , r_All(i_r), r_All(i_r), 1./(r_UE2BS_All*4*pi).*(10.^(K.*r_UE2BS_All/10)), fc, B, M, 'absolute');
    
        H_UE2BS_relative = NearFieldH(K, N, theta_All(i_theta)...
        , r_All(i_r), r_All(i_r), 1./(r_UE2BS_All*4*pi).*(10.^(K.*r_UE2BS_All/10)), fc, B, M, 'relative');

    Loss(i_theta, i_r, 1) = norm(Y_absolute_all - A*H_UE2BS_absolute, 'fro')^2; 
    Loss(i_theta, i_r, 2) = norm(Y_relative_all - A*H_UE2BS_relative, 'fro')^2;
    end  
end

xticks = 1:length(r_All);
yticks = 1:length(theta_All);

figure; grid on; 
mesh(Loss(:,:,1)/max(max(Loss(:,:,1)))); ylabel('\theta'); xlabel('r/m'); title('absolute')
colorbar

set(gca,'XTick',xticks(floor(length(r_All)/5):floor(length(r_All)/5):end),'XTickLabel',r_All(floor(length(r_All)/5):floor(length(r_All)/5):end));
set(gca,'YTick',yticks(floor(length(theta_All)/5):floor(length(theta_All)/5):end),'YTickLabel',theta_All(floor(length(theta_All)/5):floor(length(theta_All)/5):end));

figure; grid on; 
mesh(Loss(:,:,2)/max(max(Loss(:,:,2)))); ylabel('\theta'); xlabel('r/m'); title('relative')
colorbar
